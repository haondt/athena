...
class ResourceLoader:
    ...
    def load_workspace_secrets(self, root, workspace):
        ...
        success, resource = self.__try_load_file("secrets", file_path)
        if not success:
    ...
    def load_workspace_variables(self, root, workspace):
        ...
        success, resource = self.__try_load_file("variables", file_path)
        if not success:
    ...
    def __try_load_file(self, yaml_root_key, file_path):
        if file_path not in self.loaded_resources and os.path.exists(file_path) and os.path.isfile(file_path):
